...
import { getAuthorizedModalSections, filterLifecycleEventsForRole } from '../utils/visibilityMatrix';
import {
  X,
  Shield,
  Building2,
...
export const AwsDetailModal: React.FC<AwsDetailModalProps> = ({ record, onClose, userRole }) => {
  if (!record) return null;

  const role: UserRole = userRole || 'Auditor / Inspector';
  const authorizedSections = getAuthorizedModalSections(role);
  const filteredEvents = filterLifecycleEventsForRole(record.lifecycleHistory, role);
      ...
      <div className="bg-white w-full max-w-5xl rounded-xl border border-slate-300 shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-lg px-3.5 py-2 flex items-center justify-between text-xs text-slate-600">
            <div className="flex items-center gap-2">
              ...
              <span>
                Authorized Inspection Scope: Active role <strong>{role}</strong>. Restricted sections are governed by Consortium RBAC.
              </span>
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 bg-slate-50 border border-slate-200 rounded-lg p-4 text-xs">
            ...
            {authorizedSections.manufacturing && (
              <>
                <div>
                  <div className="text-slate-500 font-medium mb-1 flex items-center gap-1">
            ...
            {authorizedSections.certification && (
              <div>
                <div className="text-slate-500 font-medium mb-1 flex items-center gap-1">
            ...
            {authorizedSections.policyCompliance && (
          ...
          {authorizedSections.lifecycleHistory && (
            <div>
              ...
              <LifecycleTimeline
                ...
                currentStatus={record.lifecycleStatus}
                userRole={role}
              />
            </div>
          )}
        ...
        <div className="px-6 py-3 bg-slate-50 border-t border-slate-200 flex items-center justify-between text-xs">
          <span className="text-slate-500 font-mono">
            Digital Asset Identifier: {record.id} • Visible Events: {filteredEvents.length}
          </span>
          <button
            type="button"
            onClick={onClose}
